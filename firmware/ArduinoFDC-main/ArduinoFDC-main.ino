...
DWORD xmodem_expected_size = 0;
DWORD xmodem_bytes_written = 0;
DWORD xmodem_padding_discarded = 0;
bool xmodem_have_expected_size = false;
...
bool xmodemHandlerReceive(unsigned long no, char* data, int size)
{
  ...
  if( xmodem_have_expected_size )
    {
      DWORD remaining = (xmodem_expected_size > xmodem_bytes_written) ?
                        (xmodem_expected_size - xmodem_bytes_written) : 0;

      if( remaining < (DWORD) size )
        {
          // Ensure any excess bytes are padding (0x1A)
          for( int i = remaining; i < size; i++ )
            {
              if( (unsigned char) data[i] != 0x1A )
                {
                  xmodem_status = FR_INT_ERR;
                  return false;
                }
            }
          to_write = (UINT) remaining;
          xmodem_padding_discarded += (DWORD) (size - to_write);
        }
      else
        to_write = size;
    }
  ...
  if( to_write > 0 )
    {
      xmodem_status = f_write(&FatFsFile, data, to_write, &bw);
      if( xmodem_status != FR_OK || bw != to_write ) return false;
      xmodem_bytes_written += bw;
    }
  else
    {
      // Nothing to write but still succeed
      xmodem_status = FR_OK;
    }
              ...
              if( *p == ' ' )
                {
                  *p = '\0';
                  ...
                  if( *size_ptr == '\0' ) size_ptr = NULL;
                  break;
                }
            }

          xmodem_expected_size = 0;
          xmodem_bytes_written = 0;
          xmodem_padding_discarded = 0;
          xmodem_have_expected_size = false;

          if( size_ptr != NULL )
            {
              char *endptr = size_ptr;
              unsigned long parsed = strtoul(size_ptr, &endptr, 10);
              if( endptr != size_ptr )
                {
                  xmodem_expected_size = (DWORD) parsed;
                  xmodem_have_expected_size = true;
                }
            }

          fr = f_open(&FatFsFile, args, FA_WRITE | FA_CREATE_NEW);
          if( fr == FR_OK )
            {
              ...
              if( xmodem_have_expected_size && xmodem_status == FR_OK )
                {
                  if( xmodem_bytes_written != xmodem_expected_size )
                    {
                      Serial.println(F("\r\nSize mismatch detected."));
                      xmodem_status = FR_INT_ERR;
                    }
                  else if( xmodem_padding_discarded > 0 )
                    {
                      Serial.print(F("\r\nTrimmed "));
                      Serial.print(xmodem_padding_discarded);
                      Serial.println(F(" padding bytes."));
                    }
                }
